...
class MusicBot(discord.Client):
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    ...
                    'Use %sspawn to make the bot join your channel!' % self.config.command_prefix)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            if not listlen - drop_count:
                raise exceptions.CommandError(
                    ":frowning: No songs were added, all songs were over max duration (%ss)" % permissions.max_song_length,
                    expire_in=30
                )

            reply_text = ":notes: Added **%s** songs to be played. Position in queue: %s"
            ...
            reply_text = ":notes: Added **%s** to be played. Position in queue: %s"
            btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not songs_added:
            basetext = ":slight_frown: No songs were added, all songs were over max duration (%ss)" % permissions.max_song_length
            if skipped:
                basetext += "\nAdditionally, the current song was skipped for being too long."

            raise exceptions.CommandError(basetext, expire_in=30)

        return Response(":notes: Enqueued {} songs to be played in {} seconds".format(
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError(
                ":neutral_face: You have reached your playlist item limit (%s)" % permissions.max_songs,
                expire_in=30
            )
        ...
        search_msg = await self.send_message(channel, ":mag_right: Searching for videos...")
        await self.send_typing(channel)
        ...
        if not info:
            return Response(":exclamation: No videos found.", delete_after=30)
        ...
        for e in info['entries']:
            ...
            if not response_message:
                ...
                return Response(":frowning: Ok nevermind.", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response(":smile: Alright, coming right up!", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = ":notes: Now Playing: **%s** added by **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = ":notes: Now Playing: **%s** %s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                ':confused: There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix),
                delete_after=30
            )

    async def cmd_spawn(self, channel, author, voice_channel):
        ...
        if not chperms.connect:
            self.safe_print(":x: Cannot join channel \"%s\", no permission." % author.voice_channel.name)
            return Response(
            ...
            self.safe_print(":x: Will not join channel \"%s\", no permission to speak." % author.voice_channel.name)
            return Response(
        ...
        return Response("Hiya there! Use `n!play Song Link or Name` to start playing music! Have fun! :blush:")
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError(":thinking: Can't skip! The player is not playing!", expire_in=20)
    ...
    async def cmd_slowresponse(self):
        return Response("Having slow response times? Well here's a simple fix! Go to **Server Settings > Moderation > Explicit Message Filter**. Then select either **Don't scan any messages.** or **Scan messages from members without role.** That's it! :stuck_out_tongue_closed_eyes: ")

    async def cmd_volume(self, message, player, new_volume=None):


        if not new_volume:
            return Response('Current volume: `%s%%` :loud_sound: ' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('updated volume from %d to %d :loud_sound: ' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append(":notes: Now Playing: **%s** added by **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append(":notes: Now Playing: **%s** %s\n" % (player.current_entry.title, prog_str))
        ...
        if not lines:
            lines.append(
                ':frowning: There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix))
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response(':wastebasket: Cleaned up {} message{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
    ...
    async def cmd_leave(self, server):
        ...
        return Response(":kissing_smiling_eyes: Bai", delete_after=20)
